  ...
  try {
    ...
    try {
      ...
      if (err.code === 'ER_DUP_KEYNAME' || err.message.includes('Duplicate')) {
        console.log('   ⏭️  FK constraint already exists, skipping');
      } else {
        console.log('   ⚠️  FK constraint warning:', err.message);
      }
    }
    ...
    if (existing.length === 0) {
      ...
      `, [hashedPassword]);
      console.log('   ✅ Admin user created (email: admin@lnmiit.ac.in, password: admin123)');
    } else {
