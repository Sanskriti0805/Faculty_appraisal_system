...
const CoursesTaught = ({ initialData, readOnly }) => {
   ...
   const validateTeachingLearningRows = () => {
      ...
      const allCourses = [
         ...fallCourses.map((c, i) => ({ ...c, semText: 'Fall', rowNum: i + 1 })),
         ...springCourses.map((c, i) => ({ ...c, semText: 'Spring', rowNum: i + 1 })),
         ...summerCourses.map((c, i) => ({ ...c, semText: 'Summer', rowNum: i + 1 }))
      ]

      for (const course of allCourses) {
         const hasTitle = hasValue(course.title)
         const hasPercentage = hasValue(course.percentage)
         const hasStudents = hasValue(course.students)
         const hasFeedback = hasValue(course.feedback)
         const hasDetails = hasTitle || hasPercentage || hasStudents || hasFeedback
         const hasUpload = course.feedbackFile || course.evidence_file

         // If both empty, ignore
         if (!hasDetails && !hasUpload) continue

         // Details filled but no upload
         if (hasDetails && !hasUpload) {
            window.appToast(`Please upload Feedback Evidence for Course: ${course.title || `Row ${course.rowNum}`} in ${course.semText} Semester.`)
            return false
         }

         // Upload present but details missing
         if (hasUpload && !hasDetails) {
            window.appToast(`Please fill course details for the uploaded evidence in Row ${course.rowNum} of ${course.semText} Semester.`)
            return false
         }

         // If started, enforce all mandatory fields (standard appraisal logic)
         if (hasDetails && (!hasTitle || !hasPercentage || !hasStudents || !hasFeedback)) {
            window.appToast(`Please complete all mandatory fields (Name, %, Students, Score) for Course Row ${course.rowNum} in ${course.semText} Semester.`)
            return false
         }
      }

      const allProjects = [
         ...fallProjects.map((p, i) => ({ ...p, semText: 'Fall', rowNum: i + 1 })),
         ...springProjects.map((p, i) => ({ ...p, semText: 'Spring', rowNum: i + 1 })),
         ...summerProjects.map((p, i) => ({ ...p, semText: 'Summer', rowNum: i + 1 }))
      ]

      for (const project of allProjects) {
         const hasProjTitle = hasValue(project.projectTitle)
         const hasProjType = hasValue(project.projectType)
         const hasProjRole = hasValue(project.role)
         const hasProjStudent = hasValue(project.studentName)
         const hasProjDuration = hasValue(project.duration)
         
         const hasProjDetails = hasProjTitle || hasProjType || hasProjRole || hasProjStudent || hasProjDuration
         const hasProjUpload = project.certificateFile || project.evidence_file

         if (!hasProjDetails && !hasProjUpload) continue

         if (hasProjDetails && !hasProjUpload) {
            window.appToast(`Please upload Project Evidence for: ${project.projectTitle || `Row ${project.rowNum}`} in ${project.semText} Semester.`)
            return false
         }

         if (hasProjUpload && !hasProjDetails) {
            window.appToast(`Please fill project details for the uploaded evidence in Row ${project.rowNum} of ${project.semText} Semester.`)
            return false
         }

         if (hasProjDetails && (!hasProjTitle || !hasProjType || !hasProjRole || !hasProjStudent || !hasProjDuration)) {
            window.appToast(`Please complete all mandatory fields for Project Row ${project.rowNum} in ${project.semText} Semester.`)
            return false
         }
      }
         ...
         <table className="courses-table">
            <thead>
               <tr>
                  ...
                  <th style={{ width: '20%' }}>Course Name <span className="required-star">*</span></th>
                  <th style={{ width: '15%' }}>% of Course taught alone <span className="required-star">*</span></th>
            ...
            <tbody>
               {courses.map((course, index) => (
                  <tr key={course.id}>
                     ...
                     <td>
                        <ReadOnlyField readOnly={readOnly} value={course.feedback}>
                           <input
                              type="number"
                              step="0.001"
                           ...
                           {readOnly ? (
                              ...
                              <div className="compact-upload-wrapper">
                                 ...
                                 <label
                                    ...
                                    className={`compact-upload-btn ${course.feedbackFile || course.evidence_file ? 'has-file' : ''}`}
                                 >
                                    {course.feedbackFile || course.evidence_file ? <FileText size={18} /> : <Upload size={18} />}
                                 </label>
                     ...
                     {!readOnly && (
                        <td>
                           <button className="delete-btn" onClick={() => removeCourse(semester, index)}>
                              <Trash2 size={16} />
                           </button>
                        </td>
                     )}
